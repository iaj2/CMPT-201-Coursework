...
The client is sending 1024-byte buffer data over TCP to the server's address of
127.0.0.1:8001.
2. What data is the client sending to the server?
The client is sending five strings ("Hello", "Apple", "Car", "Green", "Dog"),
every second. Eeach string is null-padded and sent with the buffer using
write(). Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
run_acceptor takes the argument `struct acceptor_args` and contains 3 fields:
- atomic_bool run
set to false to stop the acceptor
- struct list_handle *list_handle
a pointer to the shared linked list storing all received messages
- pthread_mutex_t *list_lock
the mutex passed to each client thread to protect concurrent access to the
linked list
2. How are received messages stored?
a singly-linked list. Each message the client received is heap-allocated as a
list_node and appended to the tail. list_handle tracks the tail and total
message count.
3. What does `main()` do with the received messages?
Afther the acceptor thread joins, main verifies MAX_NUM_CLIENTS *
NUM_MSG_PER_CLIENT messages were received. It then goes through the linked list
and prints each message, freeing every node and data buffer. Finally, it
verifies collected == count and prints success or failure.
...
static void *run_client(void *args) {
  ...
  while (cargs->run) {
    ...
    if (bytes_read == -1) {
      ...
      // Safely add new_node to the list
