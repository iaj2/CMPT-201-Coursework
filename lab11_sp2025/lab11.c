...
int main() {
  ...
  // #1: Load the public key using PEM_read_PUBKEY
...
#define MAX_FILE_SIZE 512
  ...
  // #2: Check if the message is authentic using the signature.
  ...
  // 4. On the digest, init it, update it, and verify it
  EVP_DigestVerifyInit(mdctx, NULL, EVP_sha256(), NULL, pubkey);
  EVP_DigestVerifyUpdate(mdctx, message, msg_len);
  int verified = EVP_DigestVerifyFinal(mdctx, signature, sign_len);
